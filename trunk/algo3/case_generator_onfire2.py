...
def agregar_hijos(padre,arbol,desde,ciclo_tam):
    arbol.append([padre + ciclo_tam , ciclo_tam + 2*padre + 1])
    arbol.append([padre + ciclo_tam , ciclo_tam + 2*padre + 2])

def crear_arbol_binario(tam, desde,arbol,ciclo_tam):
    for i in range(tam/2):
        if i==0:
            arbol.append([desde , ciclo_tam + 1])
            arbol.append([desde , ciclo_tam + 2])

        agregar_hijos(i,arbol,desde,ciclo_tam)
...
def main(n):
    ...
    arbol = []
    llaves = []
    ciclo = ciclo_rando(habitaciones-nodos_a)
    desde  = randint(2,habitaciones-nodos_a)
    print desde
    crear_arbol_binario(nodos_a,desde,arbol,habitaciones-nodos_a)
