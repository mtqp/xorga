\begin{section}{Problema 1}
	...
	La complejidad de dicho algoritmo viene dada por: $\sum_{i=0}^{n-1} i = \frac{n*(n-1)}{2} = n^2$\\
	...
	\begin{pseudo}
		...
		\tab $max\_long\_creciente[0] \leftarrow 0$ \tab \Ode{1} \\
		\tab \FOR $i=1$ \TO $n$ \\
		...
		\tab $max\_long\_creciente[i] \leftarrow max\_long+1$\\
	\end{pseudo}
