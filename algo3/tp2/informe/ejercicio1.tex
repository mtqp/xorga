\begin{section}{Problema 1}
	...
	El algoritmo $long\_max\_creciente$ utiliza la técnica de programación dinámica, calculando para cada elemento $i$ de la secuencia original la máxima longitud de la subsecuencia creciente y decreciente que lo incluye, iterando por todos los índices $j<i$ ($\forall\; 0 \leq i < n$), sabiendo que para cada $j$ ya esta calculada la longitud de la subsecuencia creciente más larga que lo incluye.\\
	...
	Esta sumatoria, se coindice entonces con el costo de la función $long\_max\_creciente$. La cantidad de operaciones que realiza este algoritmo es \Ode{n^2}.\\
	...
	Por lo tanto, el costo del algoritmo $secuencia\_unimodal$ es: \Ode{2*n + 2*n^2} que por definición es \Ode{max(2*n, 2*n^2)} = \Ode{2*n^2} = \Ode{n^2}. \\
