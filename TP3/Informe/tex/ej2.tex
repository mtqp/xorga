...
de segmentación (dirección lineal) y esto servirá para localizar la tabla de página correspondiente.
	\item\code{Bit S}: es el \keyword{bit} 7 y determina el tamaño de la pagina, que puede ser de 4Kb o 4Mb, pero para esto último se necesita que 
el modo \code{PSE} de paginación extendida esté activado (no requerido en el trabajo actual).
	...
	\item\code{Bit D}: este \keyword{bit} deshabilita el caché, por lo que permanecerá en 0. Es el \keyword{bit} 4.
	\item\code{Bit W}: es para habilitar el modo \code{Write-through} de la \keyword{caché}, no lo utilizamos.
