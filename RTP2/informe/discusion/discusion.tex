...
\keyword{Frei-Chen} en \code{x} y descripta más arriba.
...
\begin{tabular}{|c|c|}
	...
	\hline
\end{tabular} 
}. Luego avanza una linea, desempaqueta los 16 nuevos píxeles , shiftea un byte a la izquierda y se restan a los ya acumulados, logrando así la matriz buscada. 

\subsubsection{Procesamiento en \code{y}}
	La macro \code{RobertsY} funciona de igual manera, sólo que los primeros píxeles son los que se shiftean 1 byte a la derecha y la segunda linea se 
resta a la primera sin el shift.



\subsection{Medición de Performance}

La siguiente tabla muestra la cantidad de ciclos mínima y promedio de cada implementaciçon de los filtros, obtenidos de una muestra de
